...
class _TransactionDetailsScreenState extends State<TransactionDetailsScreen> {
  ...
  Future<void> submitTransaction() async {
    try {
      ...
      if (selectedPartner == null) {
        throw Exception(
          widget.isSale
              ? "الرجاء اختيار زبون أولاً"
              : "الرجاء اختيار مورد أولاً",
        );
      }
      ...
      // إرسال عملية البيع
      final response = await http.post(
        Uri.parse(ApiEndpoints.addsale),
        headers: headers,
        body: jsonEncode(body),
      );
  ...
  Future<void> handlePayment(int? saleId) async {
    ...
    if (saleId == null) return;
    double amount = double.tryParse(_depositController.text) ?? 0;
    print("amount: $amount, method: $paymentMethod");
    if (paymentMethod == "cash") {
      if (amount > 0) {
        await sendPayment(saleId, "cash", amount);
      }
  ...
  // تم التعديل هنا لفلترة الأطراف برمجياً بحسب نوع الشاشة الحالية
  Future fetchPartners() async {
    final String type = widget.isSale ? "customer" : "supplier";
    final res = await http.get(
      Uri.parse("${ApiEndpoints.getPartners}?type=$type"),
    ...
    if (res.statusCode == 200) {
      List allPartners = parseListResponse(res.body);
      setState(() {
        // إذا كانت شاشة بيع، تعرض فقط الـ customer، وإذا كانت شراء تعرض الـ supplier
        partners = allPartners.where((p) => p['partner_type'] == type).toList();
      });
    }
  }
