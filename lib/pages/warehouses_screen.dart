...
class Warehouse {
  ...
  String name;
  String location;
  int itemsCount; // أزلت final لتحديثها يدوياً
  int categoriesCount; // أزلت final لتحديثها يدوياً
  ...
  factory Warehouse.fromJson(Map<String, dynamic> json) {
    return Warehouse(
      ...
      name: json['name'] ?? 'بدون اسم',
      location: json['location'] ?? 'بدون عنوان',
      itemsCount: 0, // قيمة افتراضية سيتم تحديثها بالطلب الثاني
      categoriesCount: 0,
    );
  }
}
...
class _WarehousesScreenState extends State<WarehousesScreen> {
  ...
  // الحل المؤقت: جلب قائمة المستودعات ثم جلب منتجات كل مستودع لحسابها
  Future<void> _fetchWarehouses() async {
    if (!mounted) return;
    setState(() => _isLoading = true);
    final prefs = await SharedPreferences.getInstance();
    ...
    try {
      // 1. جلب قائمة المستودعات الأساسية
      final response = await http.get(
        Uri.parse(ApiEndpoints.getWarehouses),
        headers: {
          "Authorization": "Bearer $token",
      ...
      if (response.statusCode == 200) {
        ...
        // 2. Loop على كل مستودع لجلب عدد منتجاته
        for (var item in data) {
          Warehouse w = Warehouse.fromJson(item);

          try {
            // طلب إضافي لجلب منتجات هذا المستودع تحديداً
            ...
            if (prodResponse.statusCode == 200) {
              ...
              // حساب إجمالي عدد القطع (ألواح الخشب)
              int totalPieces = 0;
              for (var p in products) {
                // تأكد من اسم حقل الكمية في السيرفر (quantity أو stock)
                totalPieces += (p['quantity'] as num? ?? 0).toInt();
              }

              w.itemsCount = totalPieces;
              w.categoriesCount = products.length; // عدد الأصناف المختلفة
            }
          } catch (e) {
            debugPrint("خطأ في جلب منتجات المستودع ${w.id}: $e");
          }
